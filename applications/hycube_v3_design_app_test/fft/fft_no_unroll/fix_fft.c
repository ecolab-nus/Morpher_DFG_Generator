...
__attribute__((annotate("fr:128,fi:128")))
int fix_fft_128(short fr[], short fi[], short inverse)
{
	int mr, nn, i, j, l, k, istep, n, scale, shift;
	short qr, qi, tr, ti, wr, wi;
...
void filter(int startF, int endF, fixed* real, fixed* imag, int N){
    int i;
...
main(){
	const int N = 128;
        ...
        filter(0,1,real,imag,N);
        fix_fft_128(real, imag, 1);

        printf("Filtered IFFT Samples:\n");
        for (i=0; i<N; i++){
                printf("%d: %d, %d\n", i, real[i], imag[i]);
        }

fix_fft(real,imag,0,0);
