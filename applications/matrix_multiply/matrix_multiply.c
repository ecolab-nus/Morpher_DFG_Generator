...
//printf("i,j,k: %d,%d,%d\n",i,j,k);
	   OUTPUT_MATRIX[i*C2+j] = WEIGHT_MATRIX[i*C1+k]* INPUT_MATRIX[k*C2+j]+ WEIGHT_MATRIX[i*C1+k+1]* INPUT_MATRIX[(k+1)*C2+j]+ 
							...
							WEIGHT_MATRIX[i*C1+k+20]* INPUT_MATRIX[(k+20)*C2+j]+ WEIGHT_MATRIX[i*C1+k+21]* INPUT_MATRIX[(k+21)*C2+j]+ 
	   						...
	   						WEIGHT_MATRIX[i*C1+k+30]* INPUT_MATRIX[(k+20)*C2+j]+ WEIGHT_MATRIX[i*C1+k+31]* INPUT_MATRIX[(k+31)*C2+j]+ 
	   						WEIGHT_MATRIX[i*C1+k+32]* INPUT_MATRIX[(k+22)*C2+j]+WEIGHT_MATRIX[i*C1+k+33]* INPUT_MATRIX[(k+33)*C2+j]+ 
	   						WEIGHT_MATRIX[i*C1+k+34]* INPUT_MATRIX[(k+24)*C2+j]+WEIGHT_MATRIX[i*C1+k+35]* INPUT_MATRIX[(k+35)*C2+j]+ 
	   						WEIGHT_MATRIX[i*C1+k+36]* INPUT_MATRIX[(k+26)*C2+j]+WEIGHT_MATRIX[i*C1+k+37]* INPUT_MATRIX[(k+37)*C2+j]+ 
	   						WEIGHT_MATRIX[i*C1+k+38]* INPUT_MATRIX[(k+28)*C2+j]+WEIGHT_MATRIX[i*C1+k+39]* INPUT_MATRIX[(k+39)*C2+j];
	   k=k+40;
			if(k+1 >= C1){
				k=0;
				++j;
