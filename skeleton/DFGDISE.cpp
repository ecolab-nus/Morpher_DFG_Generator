...
void DFGDISE::removeOutLoopLoad() {
...
std::vector<dfgNode*> DFGDISE::getStoreInstructions(BasicBlock* BB) {
...
int DFGDISE::handlePHINodes(std::set<BasicBlock*> LoopBB) {
...
dfgNode* DFGDISE::getStartNode(BasicBlock* BB, dfgNode* PHINode) {
...
dfgNode* DFGDISE::insertMergeNode(dfgNode* PHINode, dfgNode* ctrl, bool controlVal, dfgNode* data) {
...
dfgNode* DFGDISE::insertMergeNode(dfgNode* PHINode, dfgNode* ctrl,
...
void DFGDISE::removeDisconnetedNodes() {
...
dfgNode* DFGDISE::combineConditionAND(dfgNode* brcond, dfgNode* selcond, dfgNode* child) {
	dfgNode* temp = new dfgNode(this);
...
bool DFGDISE::checkBackEdge(dfgNode* src, dfgNode* dest) {
//	dfgNode* firstnode = NodeList[0];
//	SmallVector<std::pair<const BasicBlock *, const BasicBlock *>,8> BackedgeBBs;
//	FindFunctionBackedges(*(firstnode->getNode()->getFunction()),BackedgeBBs);
...
void DFGDISE::generateTrigDFGDOT() {
...
//	createDualInsNodes();
	...
	printDOT(this->name + "_DISEDFG.dot");
	printNewDFGXML();
}

bool DFGDISE::checkPHILoop(dfgNode* node1, dfgNode* node2) {
...
bool DFGDISE::checkBRPath(const BranchInst* BRParentBRI, const BasicBlock* currBB, bool& condVal,  std::set<const BasicBlock*>& searchedSoFar) {
	if(BRParentBRI->getSuccessor(0)==currBB){
		condVal=true;
		return true;
	}
...
void DFGDISE::scheduleCleanBackedges() {
...
void DFGDISE::fillCMergeMutexNodes() {
...
void DFGDISE::constructCMERGETree() {
...
void DFGDISE::printNewDFGXML() {


	std::string fileName = name + "_DISE_DFG.xml";
...
void DFGDISE::addOrphanPseudoEdges() {
...
dfgNode* DFGDISE::addLoadParent(Value* ins, dfgNode* child) {
	dfgNode* temp;
...
void DFGDISE::printDOT(std::string fileName) {
	std::ofstream ofs;
...
void DFGDISE::createCtrlBROrTree() {
...
void DFGDISE::createDualInsNodes() {
...
std::map<BasicBlock*, std::set<std::pair<BasicBlock*, CondVal> > > DFGDISE::getCtrlInfoBBMorePaths() {
...
std::set<std::set<dfgNode*> > DFGDISE::getCartesianProduct2Sets(std::set<std::set<dfgNode*>> a, std::set<dfgNode*> b) {
...
std::set<std::set<dfgNode*> > DFGDISE::getCartesianProductNSets(
...
int DFGDISE::commonAncCost(std::set<dfgNode*> nodes) {
