...
int write_int16_data(const char* filename, int16_t* array, int size) {
    ...
    size_t elements_written = fwrite(array, sizeof(int16_t), size, file);
...
int compare_int8(int8_t* array, const char* filename, int size) {
    FILE *file = fopen(filename, "rb");
    if (file == NULL) {
        perror("Error opening file");
        return 1;
  ...
  // Compare array with file contents
    int8_t file_byte;
    int idx = 0;
    while (fread(&file_byte, sizeof(int8_t), 1, file)) {
        if (idx >= size || file_byte != array[idx]) {
    ...
    // If the file ended but array still has elements, they are different
    if (idx < size) {
        printf("Arrays are different\n");
        fclose(file);
        return 0;
